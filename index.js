...
(async (args) => {
  ...
  const getPageAndProcessDataReceived = async (page, url, images, imageDataReceived) => {
    ...
    for (let i = 0; i < images.length; i++) {
      ...
      if (received)
        images[i].size = received.encodedDataLength > 0 ? received.encodedDataLength : received.dataLength;
    }
  }
  ...
  const badImages = images.filter(img => img.status === 404);

  logger.info(`Image stats for ${url}`);
  logger.info(`Number of images requested: ${images.length} `);
  logger.info(`Number of 404s: ${badImages.length} `);
  logger.info(`Total image size: ${Number(totalSize / 1048576).toFixed(2)} MB`);
